...
{
    //only set basic shape, size, and color
    bar.setPosition(sf::Vector2f(coordinates.x - (radius / 2), coordinates.y - radius - 20));
    ...
    bar_outline.setPosition(sf::Vector2f(coordinates.x - (radius / 2) - 2, coordinates.y - radius - 22));
...
void HealthBar::update(float max_health, float current_health, const sf::Vector2f &coordinates, float radius)
{
