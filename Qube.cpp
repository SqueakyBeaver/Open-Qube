...
void Qube::run(sf::Vector2f &run_for)
{
    ...
    if (!spinning)
    {
        if (run_for.x || run_for.y)
        {
            ...
            {
                if (run_for.x > 0)
                    angle_rotation += static_cast<float>(FacingDirections::East);
                else if (run_for.x < 0)
                    angle_rotation += static_cast<float>(FacingDirections::West);
            }
            if (run_for.y)
            {
                if (run_for.y > 0)
                    angle_rotation += static_cast<float>(FacingDirections::South);
                else if (run_for.y < 0)
                    angle_rotation += static_cast<float>(FacingDirections::North);
            }
            if (run_for.x && run_for.y)
            {
                angle_rotation /= 2.0;

                if (run_for.x < 0 && run_for.y < 0) //Whoever looks at this: this is the only way I could think of to make this work
                    angle_rotation = 315;
            }
...
void Qube::spin()
{
    ...
    if (spinning)
    {
        ...
        if (rotated_for >= 90 || !sf::Keyboard::isKeyPressed(sf::Keyboard::Key::LControl))
        {
            rotate_speed -= 1.5;
